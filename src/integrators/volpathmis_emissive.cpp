...
class EmissiveVolpathMisIntegratorImpl;
...
class EmissiveVolumetricMisPathIntegrator final
...
public:
    ...
    EmissiveVolumetricMisPathIntegrator(const Properties &props) : Base(props) {
        m_use_spectral_mis = props.bool_("use_spectral_mis", true);
        m_props            = props;
    }

    template <bool SpectralMis>
    using Impl = EmissiveVolpathMisIntegratorImpl<Float, Spectrum, SpectralMis>;
...
template <typename Float, typename Spectrum, bool SpectralMis>
class EmissiveVolpathMisIntegratorImpl final
    : public MonteCarloIntegrator<Float, Spectrum> {
...
public:
    ...
    EmissiveVolpathMisIntegratorImpl(const Properties &props) : Base(props) {}
    ...
    std::pair<Spectrum, Mask> sample(const Scene *scene, Sampler *sampler,
        ...
        for (int bounce = 0;; ++bounce) {
            ...
            Mask act_emission = false, act_null_scatter = false, act_medium_scatter = false, escaped_medium = false;
            ...
            if (any_or<true>(active_medium)) {
                ...
                // Calculate the probability of events using the analog
                // probabilities (proportional to volume parameters sigma_a and
                // sigma_s)
                Spectrum prob_emission = (mi.sigma_t - mi.sigma_s);
                Spectrum prob_scatter  =  mi.sigma_t;
                Spectrum prob_null     =  mi.sigma_n;
                ...
                Spectrum c = prob_emission + prob_scatter + prob_null;
                masked(prob_scatter, eq(c, 0.f)) = 1.f;
                masked(c, eq(c, 0.f))            = 1.f;
                prob_emission /= c;
                prob_scatter  /= c;
                prob_null     /= c;

                Mask emission_interaction = medium_sample_eta < index_spectrum(prob_emission, channel);
                Mask scatter_interaction  = medium_sample_eta < index_spectrum(prob_emission + prob_scatter, channel) && !emission_interaction;
        ...
        while (any(active)) {
            ...
            if (any_or<true>(active_medium)) {
                ...
                if (any_or<true>(is_spectral)) {
                    ...
                    masked(free_flight_pdf, mi.uniformly_sampled) = select(si.t < mi.t || mi.t > remaining_dist, 0.01f / 1.01f, 1.f / mi.m);
                    update_weights(p_over_f_nee, free_flight_pdf, tr, channel, is_spectral);
                ...
                if (any_or<true>(active_medium)) {
                    ...
                    // Calculate the probability of events using the analog
                    // probabilities (proportional to volume parameters sigma_a and sigma_s)
                    Spectrum prob_emission = (mi.sigma_t - mi.sigma_s);
                    Spectrum prob_scatter  = mi.sigma_t;
                    Spectrum prob_null     = mi.sigma_n;
    ...
    std::string to_string() const override {
        return tfm::format("EmissiveVolumetricMisPathIntegrator[\n"
                           "  max_depth = %i,\n"
                           "  rr_depth = %i\n"
                           "]",
...
MTS_IMPLEMENT_CLASS_VARIANT(EmissiveVolumetricMisPathIntegrator,
                            MonteCarloIntegrator);
MTS_EXPORT_PLUGIN(EmissiveVolumetricMisPathIntegrator,
                  "Emissive Volumetric Path Tracer integrator");
...
template <typename Float, typename Spectrum, bool SpectralMis>
Class *EmissiveVolpathMisIntegratorImpl<Float, Spectrum, SpectralMis>::m_class =
...
template <typename Float, typename Spectrum, bool SpectralMis>
const Class *
EmissiveVolpathMisIntegratorImpl<Float, Spectrum, SpectralMis>::class_() const {
    return m_class;
}

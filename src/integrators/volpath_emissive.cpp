...
class EmissiveVolumetricMisPathIntegrator final : public MonteCarloIntegrator<Float, Spectrum> {
...
template <typename Float, typename Spectrum, bool SpectralMis>
class EmissiveVolpathMisIntegratorImpl final : public MonteCarloIntegrator<Float, Spectrum> {
            ...
            if (any_or<true>(active_medium)) {
                ...
                if (any_or<true>(act_null_scatter)) {
                    ...
                    if (any_or<true>(not_spectral)) {
                       update_weights(p_over_f, mi.sigma_n / mi.combined_extinction, mi.sigma_n, channel, not_spectral && act_null_scatter);
                       update_weights(p_over_f_nee, 1.0f, mi.sigma_n, channel, not_spectral && act_null_scatter);
                    }
                ...
                if (any_or<true>(act_medium_scatter)) {
                    if (any_or<true>(is_spectral))
                        update_weights(p_over_f, mi.sigma_s / mi.combined_extinction, mi.sigma_s, channel, is_spectral && act_medium_scatter);
                    if (any_or<true>(not_spectral))
                        update_weights(p_over_f, mi.sigma_s / mi.combined_extinction, mi.sigma_s, channel, not_spectral && act_medium_scatter);
...
MTS_IMPLEMENT_CLASS_VARIANT(EmissiveVolumetricMisPathIntegrator, MonteCarloIntegrator);
MTS_EXPORT_PLUGIN(EmissiveVolumetricMisPathIntegrator, "Emissive Volumetric Path Tracer integrator");
...
template <typename Float, typename Spectrum, bool SpectralMis>
const Class* EmissiveVolpathMisIntegratorImpl<Float, Spectrum, SpectralMis>::class_() const {
    return m_class;
}
